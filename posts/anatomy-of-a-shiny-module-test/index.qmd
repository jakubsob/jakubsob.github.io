...
## {auto-animate=true background-color="#EBCA47"}

```r {source-line-numbers="" code-line-numbers=""}
describe("server", {
...
```r {source-line-numbers="3-4" code-line-numbers="3-4"}
describe("server", {
  it("should subset the data with selected variables", {
    # Arrange
    args <- list(data = iris)
...
Such modules are easier to reuse </br>and easier to compose them to build the whole app.
